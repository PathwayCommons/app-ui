const metadataParser = require('../server/graph-generation/metadataParserJson');
...
const augmentCyNodes = (biopax, cyElements) => {
  const biopaxJson = JSON.parse(biopax.replace(new RegExp('@id', 'g'), 'pathid')).biopax;

    //Get Graph Elements
    let graph = biopaxJson['@graph'];
    ...
    for (let i = 0; i < cyElements.length; i++) {
      ...
      try {
        ...
        cyElements[i].data.parsedMetadata = parsedMetadata;
      }
