...
function processXrefs(xrefList,eref,collectedData){
  ...
  for (let i = 0; i < xrefList.length; i++) {
    ...
    let refElement = biopaxMap.get(xrefList[i]);
    if (!(refElement))
      continue;
  ...
  for(let i=0;i<erefXrefList.length;i++){
    let keyName = 'Reference';
    let refElement = biopaxMap.get(erefXrefList[i]);
    if (!(refElement))
      continue;
...
function getByNodeId(id, map) {
  ...
  //TODO: finish and test this function a.s.a.p.

  //A node id (generated by the PC sbgn-converter) either have
  //a PC base prefix or identifiers.org one (both were also escaped - ':','/' etc. were
  //replaced with underscores)

  const pcBase = config.PC_XMLBASE.replace(/[^-\w.]/g,"_");
  const stdBase = new RegExp(/^http___identifiers.org_(\w+?.?\w*?)_/);

  let uri = null;
  if(id.startsWith(pcBase))
    uri = id.replace(new RegExp('^'+pcBase), config.PC_XMLBASE);
  else
    uri = id.replace(stdBase, "http://identifiers.org/$1/");

  console.log("getByNodeId, id: " + id + " => uri: " + uri);//TODO: remove

  let bpe = (map instanceof Map) ? map.get(uri) : map[uri];
  if (!bpe) {
    ...
    bpe = (map instanceof Map) ? map.get(uri) : map[uri];
  }
